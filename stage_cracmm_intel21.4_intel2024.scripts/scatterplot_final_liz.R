...
base <- "v5.5_fahim"
base2 <- ""
base3 <- "v5.5_intel2024"

cctm.file1 <- sprintf("/work/users/l/i/lizadams/CMAQ/data/data/CCTM_ACONC_v54_intel21.4_2018_12US1_%s.nc" , plotdate)
cctm.file2 <- sprintf("/work/users/l/i/lizadams/CMAQ/data/data/output_CCTM_v55_intel_STAGE_EM_2018_12US1_two_week_16x8/CCTM_ACONC_v55_intel_STAGE_EM_2018_12US1_two_week_16x8_%s.nc", plotdate)
inf.base <- nc_open(cctm.file1)

sens.list2 <- c("")
sens.list3 <- c("")
sens.list <- c("v5.5_intel2024")
...
for (lay.sens in lay.sens.list) {
  ...
  for (spc in spc.list) {
    ...
    plot_dir <- getwd()
    plot_dir1 <- sprintf("%s/plots/compiler_sens/base_%s/%s", plot_dir, base, lay.sens)
    plot_dir2 <- sprintf("%s/plots/prc_diff/base_%s/%s", plot_dir, base, lay.sens)
    plot_dir3 <- sprintf("%s/plots/m3diffout_%s/%s", plot_dir, base, lay.sens)
