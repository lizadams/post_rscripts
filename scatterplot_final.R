...
for (lay.sens in lay.sens.list) {
  ...
  for (spc in spc.list) {
    ...
    plot_dir1 <- sprintf("/overflow/ie/TRECH2/rscripts/plots/compiler_sens/base_%s/%s", base, lay.sens)
    plot_dir2 <- sprintf("/overflow/ie/TRECH2/rscripts/plots/prc_diff/base_%s/%s", base, lay.sens)
    plot_dir3 <- sprintf("/overflow/ie/TRECH2/rscripts/m3diffout", base, lay.sens)
    ...
    png(sprintf("%s/scatterplot_%s.png", plot_dir1, spc), res=150)
    plot <- xyplot(sens_val ~ base_val, groups=sens, data=subdat,
                   main=list(label=sprintf("%s vs %s Compiler Tests \nBase vs Sensitivity Values at max diff(col,row) for each timestep \n %s, %s", base, base3, spc, sens_units), cex=0.7),
    ...
    png(sprintf("%s/scatterplot_prc_diff_vs_time_%s.png", plot_dir2, spc), res=150)
    plot <- xyplot(prc_diff ~ hour, groups=sens, data=subdat,
                   main=list(label=sprintf("%s vs %s Compiler Tests \n Percentage Difference at each timestep \n %s ", base, base3, spc ), cex=0.7),
...
for (spc in unique(subdat$spc)) {
  ...
  file_path <- sprintf("%s/concdiff_base_%s_%s_%s.csv", plot_dir3, base, base3, spc)
  ...
  write.table(result, file = sprintf("%s/prc_diff_base_%s_%s_%s.csv", plot_dir3, base, base3, spc), 
              row.names = F, quote = F,col.names = TRUE)
}
if (!file.exists(sprintf("%s/total_%s_%s.csv", plot_dir3, base, base3))) {
  write.table(results, file = sprintf("%s/total_%s_%s.csv", plot_dir3, base, base3), 
